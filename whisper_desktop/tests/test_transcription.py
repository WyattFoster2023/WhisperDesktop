...
def transcription_manager(db_manager):
    manager = TranscriptionManager(db_manager=db_manager, vad_filter=False)
    yield manager
    manager.close()
...
async def test_transcription_manager_initialization(transcription_manager):
    ...
    assert transcription_manager.model is not None
...
def test_transcription_manager_basic():
    manager = TranscriptionManager(vad_filter=False)
    try:
